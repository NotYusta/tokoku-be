...
const createOptionSchema = Joi.object({
  ...
  type: Joi.string().valid("single", "multiple", "dropdown", "text").optional(),
...
const AdminProductOptionController = {
  ...
  createOption: (req: Request, res: Response) =>
    handle(res, async () => {
      const { error, value } = createOptionSchema.validate(req.body, { abortEarly: false });
      if (error) throw new ValidationError(error.details.map(d => d.message));
  ...
  updateOption: (req: Request, res: Response) =>
    handle(res, async () => {
      ...
      const { error, value } = updateOptionSchema.validate(req.body, { abortEarly: false });
      if (error) throw new ValidationError(error.details.map(d => d.message));
